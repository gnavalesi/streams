...
trait Solver extends GameDef {
	...
	def from(initial: Stream[(Block, List[Move])],
		...
		case p #:: ps => {
			val neighbors = neighborsWithHistory(p._1, p._2)
			val onlyNew = newNeighborsOnly(neighbors, explored + p._1)

			onlyNew ++ from(ps ++ onlyNew, explored + p._1)
		}
	}
